...
gather_global_view_date <- function(con = R_CON_DB, team_id) {
  get_tbl(table = "game_global") %>%
    tfilter_view(con = con, team_id = team_id) %>%
    rename(Game_id = index) %>%
    inner_join(get_tbl(table = "Games")) %>%
    select(-Opp_id, -Position) %>%
    collect() %>%
    gather("metric", "val", -Player_id, -Game_id, -Date)
}
...
get_all_views <- function(con = R_CON_DB, team_id) {
  list(
    ...
    game_global = get_tbl(con, table = "game_global") %>%
      tfilter_view(con = con, team_id = team_id) %>%
      gather_global_view(),
    game_date_global = gather_global_view_date(con = con, team_id = team_id))
}
